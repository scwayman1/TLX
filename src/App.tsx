...
function InvestigationWorkspace({investigation,setInvestigation,onClose,onAction,onReset}:{investigation:Investigation;setInvestigation:(value:Investigation)=>void;onClose:()=>void;onAction:(message:string)=>void;onReset:()=>void}) {
 ...
 const recordDecision=()=>{
   if (!rationale.trim()) { setRationaleError('Decision rationale is required'); return }
   setRationaleError('')
   setRecorded(recordDecisionPlan(plan,{optionId:plan.selectedOptionId ?? 'expedite',rationale,actorId:'user-dfoster',workOrderIntentId:'WO-24091',idempotencyKey:`decision:WO-24091:${plan.selectedOptionId ?? 'expedite'}:v1`}))
 }
