...
import { approveToolRun, createAssetRiskInvestigation, executeApprovedToolRun, rejectToolRun, proposeToolRun, appendDecisionToInvestigation, SYNTHETIC_ACTIVE_ACTOR_ID, type Investigation } from './domain/investigation'
import { DecisionPlanError } from './domain/decision-plan'
...
function InvestigationWorkspace({investigation,setInvestigation,onClose,onAction,onReset}:{investigation:Investigation;setInvestigation:(value:Investigation)=>void;onClose:()=>void;onAction:(message:string)=>void;onReset:()=>void}) {
 ...
 const recordDecision=()=>{
   ...
   if (hasError) return
   try {
     // The recorded actor is the bounded synthetic active actor shown in this flow — no hidden substitution.
     const result=recordDecisionPlan(plan,{optionId:plan.selectedOptionId as 'expedite'|'reschedule',rationale,actorId:SYNTHETIC_ACTIVE_ACTOR_ID,workOrderIntentId:'WO-24091',idempotencyKey:`decision:WO-24091:${plan.selectedOptionId}:v1`})
     setRecorded(result)
     setInvestigation(appendDecisionToInvestigation(investigation,result.decisionPlan))
   } catch (error) {
     // Typed domain errors map to distinct, correct messages at this control.
     // Unexpected integrity failures surface visibly as their own class.
     if (error instanceof DecisionPlanError && error.code==='idempotency_conflict') {
       setOptionError('Recorded decision conflicts with this idempotency key; reset the demo to replay')
     } else if (error instanceof DecisionPlanError) {
       setRationaleError(error.message)
     } else {
       throw error
     }
   }
