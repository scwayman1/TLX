 ...
 return <div className="page">
  ...
  <div className="content-grid">
    <section className="panel priorities"><div className="panel-head"><div><h2>Priority queue</h2><p>Only work that needs a person’s attention</p></div><button className="link" onClick={()=>onPage('Work orders')}>View all <ChevronRight size={14}/></button></div>
      <div className="priority-card critical"><div className="priority-icon"><AlertTriangle size={18}/></div><div><span><Badge tone="danger">Safety critical</Badge><small>TRL-443 · Phoenix Yard</small></span><h3>Brake failure is blocking field deployment</h3><p>Investigate the risk with grounded evidence, visible tool use, and human approval.</p><div className="decision"><div className="avatars">SW</div><span><b>Decision owner</b>{SYNTHETIC_PERSONA.name} · due in 1h 24m</span></div></div><div className="card-actions"><button className="secondary" onClick={()=>onAsset(assets[3])}>Review asset</button><button className="primary" onClick={onInvestigate}>Investigate with agent</button></div></div>
...
function DomainPage({page,onAction}:{page:string;onAction:(m:string)=>void}) { const info:Record<string,{icon:any;desc:string;stats:string[]}>={
 ...
 const queues:Record<string,[string,string,string,string][]>={
  ...
  'Controls':[['Review','Expedited-parts approval threshold','Owner · Scott Wayman (synthetic)','warning'],['Passing','Asset status transition policy','24 cases evaluated','success'],['Draft','Agent recommendation boundary','Awaiting accountable owner','neutral']]}
