...
- **Identity coherence:** Scott Wayman is the single active synthetic persona and the required approver for both options. Every surface that displays the persona's name (mission-control decision-owner card, thread turn, approval box, option approver lines, recorded panel, Controls queue owner row) derives name and qualifier from the fixture, and the approver display derives from the frozen policy, not duplicated strings. The Operating Memory timeline renders raw actor IDs as audit attribution (`user-scott`, `agent-operations`, `tool-simulator`), by design. Dana Foster remains only the WO-24091 repair assignee.
...
- **`recordDecisionPlan` validation order (fail-closed):** (1) key grammar + key↔work-order consistency → (2) canonical option existence (frozen canon *and* present in plan) → (3) authorization against the frozen `SYNTHETIC_APPROVER_POLICY` (never from plan/event state) → (4) non-empty trimmed rationale → (5) full ledger validation: every prior event must be one this module could itself have accepted (canonical ID and key, canonical option, authorized actor, non-empty rationale, fixed demo-clock timestamp) with unique IDs → (6) only then: same key + equal payload = no-op replay returning a fully populated plan; same key + different payload = `idempotency_conflict`; new key whose derived ID collides = `audit_integrity`.
- **Operating Memory integration** `integrateDecisionEvents(investigation, events)`: applies the same full per-event ledger validation as the record path, including uniqueness *within the incoming batch* (a ledger with duplicate IDs is corrupt and never partially integrated), integrates **all** unintegrated events deterministically in ledger order, is idempotent (timeline entry ID `decision.recorded-<event.id>`; re-integration is a no-op), and fails closed (`audit_integrity`) on an ID collision with different content. Never truncates to the last event. The UI commits its "Decision recorded" view only after integration succeeds — a failed audit write is never a silent success.
